...
async function logOrder(
  ...
  try {
    ...
    // If API fails, order was still sent via WhatsApp
  }
}
...
export default function CartDrawer({ mostrador = false }: { mostrador?: boolean }) {
  ...
  const { zoneResult, setZoneResult } = useZone();

  const isMostrador = mostrador;

  // Stepper state (non-mostrador only)
  const [step, setStep] = useState<'review' | 'delivery' | 'payment'>('review');
  ...
  // Reset stepper and pre-fill zone when drawer opens
  useEffect(() => {
    if (isOpen) {
      setStep('review');
      // Pre-fill zone from context if not yet set
      if (zoneResult && selectedZone === null) {
        setSelectedZone(zoneResult.zone);
        if (zoneResult.activeCoords && zoneResult.zone === 'delivery') {
          setAddrCoords({ lat: zoneResult.activeCoords.lat, lng: zoneResult.activeCoords.lng, display: zoneResult.addrDisplay });
          setAddrInput(zoneResult.addrDisplay);
          setMode('manual');
        }
      }
    }
  ...
  /* ── Save zone result to context when continuing to payment ── */
  function goToPayment() {
    ...
    if (selectedZone) {
      setZoneResult({
        zone: selectedZone,
        distKm,
        deliveryFee: selectedZone === 'retiro' ? 0 : (deliveryFee ?? null),
        addrDisplay: addrCoords?.display ?? addrInput,
        activeCoords,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Persist waNumber so confirmation page can build WA link
      try { localStorage.setItem('gustoso_wa', waNumber); } catch {}
      await logOrder(items, finalTotal, orderId, finalLocationUrl, paymentMethod ?? undefined, appliedDiscount?.code, deliveryFee ?? undefined);
  ...
  /* ── Step label helper ── */
  const stepNum   = step === 'review' ? 1 : step === 'delivery' ? 2 : 3;
  const stepTitle = step === 'review' ? 'Revisá tu pedido' : step === 'delivery' ? '¿Cómo lo recibís?' : 'Pago y confirmar';
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:600, display:'flex', flexDirection:'column', justifyContent:'flex-end', alignItems:'center' }}>
      ...
      <div style={{ position:'relative', zIndex:1, width:'100%', maxWidth:'var(--max)', background:'var(--card)', borderRadius:'var(--radius) var(--radius) 0 0', maxHeight:'85dvh', display:'flex', flexDirection:'column', animation:'slideUp .3s ease', boxShadow:'0 -8px 40px rgba(0,0,0,0.3)' }}>
        ...
        {/* ── MOSTRADOR: full original layout ── */}
        {isMostrador && items.length > 0 && (
          <>
            <div style={{ overflowY:'auto', flex:1, padding:'12px 20px' }}>
        ...
        {!isMostrador && items.length > 0 && (
          <>
            ...
            <div style={{ padding:'14px 20px 32px', borderTop:'1px solid var(--border)', flexShrink:0 }}>
              ...
              {/* Step 2 footer: continue to payment */}
              {step === 'delivery' && (
                <button
                  onClick={goToPayment}
              ...
              {/* Step 3 footer: total + send */}
              {step === 'payment' && (
                <>
                  <TotalBreakdown total={total} appliedDiscount={appliedDiscount} discountAmount={discountAmount} deliveryFee={deliveryFee} finalTotal={finalTotal} selectedZone={selectedZone} />
                  <button onClick={handleSend} disabled={sending}
                    style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'center', gap:10, background: sending ? '#1aaa52' : '#25D366', color:'#fff', padding:'15px 24px', borderRadius:999, border:'none', fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:20, letterSpacing:.5, cursor: sending ? 'not-allowed' : 'pointer', boxShadow:'0 4px 16px rgba(37,211,102,0.3)' }}>
...
function TotalBreakdown({
  total, appliedDiscount, discountAmount, deliveryFee, finalTotal, selectedZone,
}: {
  total: number;
  ...
  finalTotal: number;
  selectedZone: 'retiro' | 'delivery' | null;
}) {
  return (
